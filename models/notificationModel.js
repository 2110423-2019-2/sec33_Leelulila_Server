...
exports.jobNotify = catchAsync(async (mongo, jobId, email, type) => {
...
exports.notifyIncomingJob = catchAsync(async (mongo) => {
...
exports.notifyMany = catchAsync(async (mongo, email, msg) => {
  const payload = {
    timestamp: Date.now(),
    string: msg,
    status: 0,
...
exports.notifyPayload = catchAsync(async (mongo, email, payload) => {
  const result = await mongo
    .db('CUPartTime')
    .collection('Users')
    .updateMany({
