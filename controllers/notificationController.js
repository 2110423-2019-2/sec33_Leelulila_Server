...
exports.jobNotify = catchAsync(async (jobId, email, type) => {
    ...
    if (type == 0) {
        ...
        const string = "Sadly, your application for " + jobName + " has been refused, good luck next time!"
        payload = {
            "timestamp": Date.now(),
            "string": string,
            "status": 0
        ...
        const string = "Congratulations you has been accepted to " + jobName + " job"
        payload = {
            "timestamp": Date.now(),
            "string": string,
            "status": 0
...
exports.notifyIncomingJob = catchAsync(async () => {
            ...
            if (jobs.job.CurrentAcceptedEmployee.length > 0) {
                ...
                if (jobTime - now > 0) {
                    ...
                    if (length < 259200000 && length > 86400000) {
                        ...
                        await mongo.db("CUPartTime").collection("Job").updateOne({
                            _id: jobs._id
                        }, {
                            $pull: {
                                notify1: {
                        ...
                        const result3 = await mongo.db("CUPartTime").collection("Users").updateMany({
                            email: {
                                $in: emails
                            }
                        }, {
...
exports.notifyMany = catchAsync(async (email, msg) => {
    const payload = {
        "timestamp": Date.now(),
        "string": msg,
        "status": 0
...
exports.notifyPayload = catchAsync(async (email, payload) => {
    const result = await mongo.db("CUPartTime").collection("Users").updateMany({
        email: {
            $in: email
        }
