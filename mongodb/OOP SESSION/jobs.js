...
async function updateJobEmployeeByEmail(client, id, email, res) {
  ...
  try {
    ...
    if (find) {
      ...
      let jobNoti = new JobNotification(0, id);
      jobNoti.notify(client, find.job.Employer);
      console.log(
        `${result.matchedCount} document(s) matched the query criteria.`
      );
      console.log(`${result.modifiedCount} document(s) was/were updated.`);
...
async function updateJobAcceptedEmployeeByEmail(client, id, email, res) {
  try {
    ...
    if (find) {
      ...
      let jobNoti = new JobNotification(2, id);
      jobNoti.notify(client, email);
      console.log(
        `${result.matchedCount} document(s) matched the query criteria.`
      );
      console.log(`${result.modifiedCount} document(s) was/were updated.`);
...
async function deleteJobByID(client, id, res) {
  try {
    ...
    if (result) {
      ...
      // );
      let deletedNoti = new Notification(find.job.JobName + ' which you applied has been deleted');
      deletedNoti.notify(client, pendingList);
      // notify.notifyMany(
      //   client,
      //   acceptedList,
      //   find.job.JobName + ' has been deleted'
      // );
      let deletedNotiEmployer = new Notification(find.job.JobName + ' has been deleted');
      deletedNotiEmployer.notify(client, acceptedList);
...
async function deleteCurrentEmployeeByID(client, jobID, email, res) {
  try {
    ...
    if (find.job.CurrentEmployee != null) {
      ...
      let jobNoti = new JobNotification(1, jobID);
      jobNoti.notify(client, email);
